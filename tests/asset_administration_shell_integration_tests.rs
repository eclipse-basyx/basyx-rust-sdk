...
fn add_one_gives_len_one() {
    // arrange
    let (sm1, _, _) = setup();
    let mut ut = AssetAdministrationShell::new("".into(), AssetInformation::new(AssetKind::Instance));
    
    // act
    ut.add_reference_to_submodel(&sm1);
    
    // assert
    assert_eq!(ut.submodels.expect("AssetAdministrationShell/submodels is None").len(), 1);
}
...
fn add_three_gives_len_three() {
    ...
    // assert
    assert_eq!(ut.submodels.expect("AssetAdministrationShell/submodels is None").len(), 3);
}
...
fn add_three_and_delete_middle_gives_len_two() {
    ...
    // assert
    assert_eq!(ut.submodels.expect("AssetAdministrationShell/submodels is None").len(), 2);
}